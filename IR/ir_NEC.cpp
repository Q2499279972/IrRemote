...
bool  IRrecv::decodeNEC(decode_results *results)
{
  ...
  {
    // Check data "mark"
    if(!matchMark(results->rawbuf[offset], NEC_BIT_MARK))  return false ;
    offset++;
    // Suppend this bit
    if(matchSpace(results->rawbuf[offset], NEC_ONE_SPACE))  data = (data << 1) | 1 ;
    else if(matchSpace(results->rawbuf[offset], NEC_ZERO_SPACE))  data = (data << 1) | 0 ;
    else                                                            return false ;
    offset++;
  }
  // Success
